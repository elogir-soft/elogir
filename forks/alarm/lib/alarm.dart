...
class Alarm {
  ...
  /// Stream of alarm IDs that were snoozed via the notification action.
  static final _snoozedStream = StreamController<int>.broadcast();

  /// Stream that emits the native alarm ID whenever a snooze action is
  /// triggered from the notification.
  static Stream<int> get snoozed => _snoozedStream.stream;

  static Future<void> _alarmSnoozed(int alarmId) async {
    _ringing.add(_ringing.value.removeById(alarmId));

    // The native side already rescheduled the alarm, so reload it from
    // storage and add to scheduled set.
    final alarm = await getAlarm(alarmId);
    if (alarm != null) {
      _scheduled.add(_scheduled.value.add(alarm));
    }

    _snoozedStream.add(alarmId);
    updateStream.add(alarmId);
  }
}
