...
class AlarmService : Service() {
    ...
    fun handleSnoozeAlarmCommand(context: Context, alarmId: Int) {
        if (alarmId == 0) return

        // Get current alarm settings from storage.
        val alarm = alarmStorage?.getSavedAlarms()?.find { it.id == alarmId }
        if (alarm == null) {
            Log.e(TAG, "Cannot snooze alarm $alarmId: not found in storage.")
            unsaveAlarm(alarmId)
            return
        }

        val snoozeMins = alarm.notificationSettings.snoozeDurationMinutes
        val snoozeTimeMs = System.currentTimeMillis() + snoozeMins * 60 * 1000L
        val snoozedAlarm = alarm.copy(dateTime = Date(snoozeTimeMs))

        // Save snoozed alarm to storage and schedule via AlarmManager.
        alarmStorage?.saveAlarm(snoozedAlarm)
        scheduleAlarmViaManager(context, snoozedAlarm, snoozeTimeMs)

        // Notify Flutter about the snooze.
        AlarmPlugin.alarmTriggerApi?.alarmSnoozed(alarmId.toLong()) {
            if (it.isSuccess) {
                Log.d(TAG, "Alarm snoozed notification for $alarmId processed by Flutter.")
            } else {
                Log.d(TAG, "Alarm snoozed notification for $alarmId encountered error in Flutter.")
            }
        }

        // Stop ringing (audio, vibration, foreground).
        stopAlarm(alarmId)
    }

    private fun scheduleAlarmViaManager(context: Context, alarm: AlarmSettings, triggerTimeMs: Long) {
