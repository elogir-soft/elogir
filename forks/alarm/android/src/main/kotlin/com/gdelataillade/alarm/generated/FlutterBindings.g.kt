...
)
 {
  companion object {
    fun fromList(pigeonVar_list: List<Any?>): NotificationSettingsWire {
      ...
      val snoozeDurationMinutes = pigeonVar_list[10] as Long
      return NotificationSettingsWire(title, body, stopButton, icon, iconColorAlpha, iconColorRed, iconColorGreen, iconColorBlue, keepNotificationAfterAlarmEnds, snoozeButton, snoozeDurationMinutes)
    }
  }
  fun toList(): List<Any?> {
    return listOf(
  ...
  fun alarmStopped(alarmIdArg: Long, callback: (Result<Unit>) -> Unit)
{
  ...
  fun alarmSnoozed(alarmIdArg: Long, callback: (Result<Unit>) -> Unit)
{
    val separatedMessageChannelSuffix = if (messageChannelSuffix.isNotEmpty()) ".$messageChannelSuffix" else ""
