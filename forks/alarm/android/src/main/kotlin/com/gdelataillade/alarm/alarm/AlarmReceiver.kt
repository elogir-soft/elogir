...
class AlarmReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        /// Snooze alarm from notification snooze button.
        if (action == ACTION_ALARM_SNOOZE) {
            val id = intent.getIntExtra("id", 0)
            Log.d("AlarmReceiver", "Received snooze alarm command from notification, id: $id")
            AlarmService.instance?.let {
                it.handleSnoozeAlarmCommand(context, id)
                return
            }
        }
